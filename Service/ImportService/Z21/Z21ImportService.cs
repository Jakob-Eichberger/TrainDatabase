...
namespace Service.ImportService.Z21
{
  public class Z21ImportService
  {
                     ...
                     async () =>
                     {
                       if (z21File.Extension.ToLower() is not ".z21")
                       {
                         throw new NotSupportedException(
                                                         $"Importing a {z21File.Extension} file is not supported!");
                       }
                       ...
                       using SqliteConnection con = new($"Data Source={filePath}");
    ...
    private async Task MapFunctionsAsync(SqliteConnection con, bool removeEmptyFunctions)
    {
      ...
      List<FunctionModel> functions = f.Select(
                                               e => new FunctionModel()
                                                    {
                                                      Id = (int)e.id,
                                                      Vehicle = Database.Vehicles.FirstOrDefault(
                                                                                                 v => v.Id == (int)e
                                                                                                       .vehicle_id),
                                                      ButtonType = (ButtonType)(int)e.button_type,
                                                      Name =
                                                        e.shortcut.IsNullOrWhiteSpace() ? e.image_name : e.shortcut,
    ...
    private async Task MapVehiclesAsync(SqliteConnection con)
    {
      ...
      List<VehicleModel> vehicles = v.Select(
                                             e => new VehicleModel()
                                                  {
                                                    Id = (int)e.id,
                                                    Name = e.name,
      ...
      await Database.SaveChangesAsync();
      await Database.Vehicles.Where(e => string.IsNullOrWhiteSpace(e.Name))
                    .ForEachAsync(e => Log.Warning($"Imported Vehicle with Adresse {e.Address} has no display name!"));
    }
      ...
      return await Task.Run(
                            () =>
                            {
                              ...
                              string zipFileLocation =
                                new StringBuilder(z21Path).Replace(".z21", ".zip").ToString();
                              ...
                              string sqlLiteDB =
                                files.FirstOrDefault(e => Path.GetExtension(e).ToLower() == ".sqlite") ??
                                throw new FileNotFoundException("Failed to find the required .sql file!");

                              foreach (string image in files.Where(e => Path.GetExtension(e) != ".sqlite")
                                                            .ToList())
                              {
                                File.Move(
                                          image,
                                          $"{Configuration.ApplicationData.VehicleImages.FullName}\\{Path.GetFileName(image)}");
                              }
