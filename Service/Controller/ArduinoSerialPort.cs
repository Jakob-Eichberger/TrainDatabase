...
namespace Service.Controller
{
  ...
  public sealed class ArduinoSerialPort : SerialPort, IDisposable
  {
    ...
    {
      ResetTimer();
      foreach (char item in Regex.Replace(ReadExisting().Trim(), @"\s+", string.Empty).ToCharArray())
      {
    ...
    {
      DataReceived?.Invoke(this, new(string.Join("", SerialBuffer.ToList())));
      ReleaseSemaphore();
    }
