    ...
    public class TimeCapture
    {
        ...
        private async Task<decimal> GetTimeBetweenSensors(int speed, bool direction)
        {
            try
            {
                ...
                return decimal.TryParse(data.Replace(',', '.'), NumberStyles.Any, CultureInfo.InvariantCulture, out decimal result) ? result : throw new ApplicationException($"Serial bus data ('{data}') could not be parsed as decimal!");
            }
            finally
            {
        ...
        {
            var temp = Database.Vehicles.FirstOrDefault(e => e.Id == VehicleModel.Id) ?? throw new Exception($"Fahrzeug mit Adresse '{VehicleModel.Address}' nicht gefunden!");
